...
TEST(RMQHuffmanTest, GetMicrotreeInorderTest) {
    ...
    for (int64_t i = 0; i < microtrees.size(); i++) {
...
TEST(RMQHuffmanTest, Select1Rank1Test) {
    ...
    for (int64_t i = 0; i < chunks.size(); i++) {
        ...
        for (int64_t j = 0; j < clen; j++) {
            auto bit = (cbp >> j) & 1;
            if (bit) {
                std::pair<uint64_t, uint64_t> select_expected = {i, j};
                std::pair<uint64_t, uint64_t> select_actual =
