...
void benchmarkFerrada(const vector<int>& perm,
    ...
    time_keeper("construction", 0);
    RMQRMM64 rmq(const_cast<int*>(perm.data()), perm.size());
    time_keeper("construction", 1);
...
// void benchmarkSDSL(const vector<int>& perm,
//                    const vector<pair<int, int>>& queries) {
//     cout << "SDSLRMQ" << endl;
...
//     time_keeper("construction", 0);
//     sdsl::rmq rmq(perm.data(), perm.size());
//     time_keeper("construction", 1);
...
//     benchmark_result.push_back({"memory", 8.0 * rmq.getSize() /
//     perm.size()});

//     for (auto&& [label, value] : benchmark_result) {
//         cout << left << setw(20) << (label + ":") << right << setw(20) <<
//         value
//              << endl;
//     }
// }
